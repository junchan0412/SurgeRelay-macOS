...
Web management HTTP primitives belong in `WebManagementHTTP.swift`; API authentication, same-origin validation, and session cookie rules belong in `WebRequestSecurity.swift`; request authentication throttling belongs in `WebAuthenticationThrottle.swift`; connection lifecycle and event streaming stay in `WebManagementServer.swift`. Keep these boundaries separate so parser, authentication, security headers, rate limiting, and socket lifecycle can be tested and changed independently.

Cached module metadata refresh and successful-conversion module state planning belong in `ModuleMetadataRefreshPlanner`. Keep restored Script-Hub subscription metadata, source revision writeback, conversion-engine revision decisions, override base-hash/conflict decisions, preferred icon selection, icon-cache refresh decisions, detected source-format updates, content-change detection, and success history text there. AppModel should read cached files or conversion outputs and then apply the returned module/icon plan.
