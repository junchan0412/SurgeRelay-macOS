...
- Main state owner: `SurgeRelay/AppModel.swift`; automatic publish scheduling lives in `SurgeRelay/AppModel+AutomaticPublishing.swift`, credentials live in `SurgeRelay/AppModel+Credentials.swift`, diagnostics live in `SurgeRelay/AppModel+Diagnostics.swift`, GitHub connection testing lives in `SurgeRelay/AppModel+GitHubConnectivity.swift`, GitHub publish preparation/execution lives in `SurgeRelay/AppModel+GitHubPublishing.swift`, local module scanning/import orchestration lives in `SurgeRelay/AppModel+LocalModules.swift`, local `file://` source watching and change-driven sync lives in `SurgeRelay/AppModel+LocalSourceSync.swift`, module derived state lives in `SurgeRelay/AppModel+ModuleState.swift`, module output-folder menu and remote-directory refresh orchestration lives in `SurgeRelay/AppModel+ModuleOutputFolders.swift`, module mutation and ordering lives in `SurgeRelay/AppModel+Modules.swift`, combined-output rebuild, local export, legacy cleanup, and shared publish-file assembly live in `SurgeRelay/AppModel+PublishedOutput.swift`, preview/address read access lives in `SurgeRelay/AppModel+PreviewAccess.swift`, preview edit orchestration lives in `SurgeRelay/AppModel+PreviewEditing.swift`, manual publish UI orchestration lives in `SurgeRelay/AppModel+Publishing.swift`, Script-Hub engine refresh orchestration lives in `SurgeRelay/AppModel+ScriptHub.swift`, settings forwarding lives in `SurgeRelay/AppModel+Settings.swift`, module update completion orchestration lives in `SurgeRelay/AppModel+UpdateCompletion.swift`, module update loop orchestration lives in `SurgeRelay/AppModel+Updates.swift`, Web-management token/server wiring lives in `SurgeRelay/AppModel+WebManagement.swift`, and foreground work/cancellation/history helpers live in `SurgeRelay/AppModel+Work.swift`
...
- Local/GitHub publishing: `SurgeRelay/AppModel+Publishing.swift`, `SurgeRelay/AppModel+GitHubPublishing.swift`, `SurgeRelay/AppModel+AutomaticPublishing.swift`, `SurgeRelay/Services/PublishCoordinator.swift`, `SurgeRelay/Services/PublishFileAssembler.swift`, `SurgeRelay/Services/ModuleFileStore.swift`, `SurgeRelay/Services/GitHubClient.swift`
...
- Module and combined-module preview panes plus override comparison belong in `ModulePreviewViews.swift`; the AppKit `NSTextView` code/highlight bridge belongs in `ModuleCodeTextView.swift`; the editor's find/replace/go-to-line state and menu command target resolution belong in `ModuleCodeEditorController.swift`; the SwiftUI find bar and the undo/redo/find button group belong in `ModuleCodeSearchBar.swift`; small reusable visual primitives stay in `Components.swift`.
- `SettingsView.swift` owns the settings window shell, tab selection, credential preloading, and QR sheet routing. General settings belong in `SettingsGeneralView.swift`; local/GitHub publishing settings and local root diagnostics belong in `SettingsPublishingView.swift`; credentials-specific token UI belongs in `SettingsCredentialsView.swift`; Web service access UI belongs in `SettingsWebManagementView.swift`; installation/update diagnostics and export live in `SettingsDiagnosticsView.swift`; reusable settings chrome and rows belong in `SettingsComponents.swift`.

## Model Boundaries

Keep cross-service models in focused files rather than reintroducing a shared catch-all model file. Conversion output and generated assets belong in `ConversionModels.swift`; shared module counts belong in `ModuleCollectionSummary.swift`; source revision snapshots belong in `SourceRevisionModels.swift`; app-wide user-facing errors belong in `RelayError.swift`; publishing value types such as `PublishFile`, `PublishReport`, and `PublishPreview` belong in `PublishModels.swift`; update history and latest GitHub publish snapshots belong in `UpdateHistoryModels.swift`; GitHub Release payloads and release-update URLs belong in `GitHubReleaseModels.swift`; publishing coordination belongs in `PublishCoordinator.swift`; update coordination belongs in `UpdateCoordinator.swift`; Web management URL/access presentation belongs in `WebManagementController.swift`; configuration-directory migration belongs in `ConfigurationManager.swift`; work/task state and update-admission rules belong in `WorkActivity.swift`; persistent settings belong in `AppSettings.swift`; module identity and source/storage relationships belong in `RelayModule.swift`; add/edit draft state belongs in `ModuleDraft.swift`; output-folder and output-path inspection helpers belong in `ModuleOutputPath.swift`; main-window module search text, content-cache keys, content-index rebuild planning, and preview-content loading decisions belong in `ModuleSearchIndex.swift`.
...
Published file assembly belongs in `PublishFileAssembler`: it owns adding the combined module data, materializing standalone modules with argument overrides, applying Surge metadata, appending generated assets, and consulting the local self-overwrite check. `AppModel+PublishedOutput.swift` owns the shared call into the assembler plus combined-output rebuild, local export, and legacy cleanup orchestration. Keep `AppModel+Publishing.swift` focused on GitHub previews, commits, and persisted publish manifests.
...
GitHub publish result planning belongs in `GitHubPublishPlanner`. Keep repository privacy metadata update decisions, target descriptions, preview payload construction, publish preparation validation, stale-path candidates, path-plan persistence policy, selected-publish path merging, no-files error recognition, no-change/no-files status text, success messages, and update-history entry construction there. `AppModel+GitHubPublishing.swift` should own token checks, repository privacy probes, actual GitHub client calls, and persistence of the returned path plan; `AppModel+Publishing.swift` should stay focused on user-facing publish, preview, confirmation, and status orchestration.
...
Surge module sanitizing belongs in `SurgeModuleSanitizer.swift`: keep line-ending normalization, empty Body Rewrite removal, converted Loon Map Local script extraction, and generated Script section insertion there. `ModuleMerger.swift` should focus on parsing sections, ordering sections, merging key/value directives, and building the combined module output.

Module source and storage enums belong in `ModuleSourceModels.swift`: keep source identity canonicalization, source format detection, storage-location display metadata, source-origin titles, and update-state titles there. `RelayModule.swift` should stay focused on persisted module fields, decoding migration, subscription metadata application, and derived published paths.

GitHub REST request plumbing belongs in `GitHubRESTClient.swift`: keep API URL construction, GitHub headers, repository visibility checks, JSON request/response handling, and contents SHA lookups there. `GitHubClient.swift` should focus on publish validation, remote tree snapshots, diffing desired files against GitHub, commit/tree/ref writes, and retrying reference conflicts.
...
Local `file://` module source watching belongs in `LocalSourceWatcher` and `LocalSourceSyncPlanner`. Keep the FSEvents stream lifecycle in the watcher, and keep watched-directory derivation, file-source selection, content hashing, changed-module detection, and user-visible sync status in the planner. Only modules whose resolved update source is a local file are watched: a local module carrying Script-Hub `#SUBSCRIBED` metadata updates from its remote `originalURL`, so its changes belong to the source revision check instead.

`LocalSourceWatcher`'s FSEvents callback must stay a file-scope function, not a closure inside the `@MainActor` class: a closure inherits the method's actor isolation, Swift 6 inserts an executor assertion, and the callback runs on a background dispatch queue, so the assertion traps with SIGTRAP on the first file event. Its `Unmanaged` bridge object must also stay a file-scope type for the same reason. Watched paths go through `realpath` because FSEvents only observes literal paths — a path reached through a symlink registers successfully and then never reports anything; do not switch to `URL.resolvingSymlinksInPath()`, which folds `/private/tmp` back to `/tmp` on macOS.

The change decision must stay the sha256 of the source file, matching `SourceRevisionService`, so repeated scans are idempotent — a successful conversion writes the same hash back to `sourceContentHash`. `AppModel+LocalSourceSync.swift` owns only watcher lifecycle, event debouncing, and calling `updateAll(only:refreshesScriptHubEngine:)`; it passes `refreshesScriptHubEngine: false` so a local file save does not also probe the upstream Script-Hub engine. FSEvents is created with `kFSEventStreamCreateFlagIgnoreSelf` so Surge Relay's own published output does not feed back as a source change. App launch also runs one scan because changes made while the app was closed produce no FSEvents notification.

Code editor text behavior belongs in `CodeSearchEngine` and `CodeEditorTextTransform`. Keep literal/regular-expression matching, wrap-around next/previous selection, single and whole-document replacement, and match summary text in the search engine; keep indent, unindent, indentation-preserving newline, comment toggling, and line lookup in the transform. Both work on UTF-16 offsets so selections stay correct in modules containing Chinese text.

Every programmatic edit must go through `CodeTextView.applyEdit(_:)`, which brackets the change with `shouldChangeText(in:replacementString:)` and `didChangeText()`. Editing `textStorage` directly leaves the change out of the undo stack, which is what previously made ⌘Z ignore indent, comment, and replace operations. The editor owns its own `UndoManager` through `undoManager(for:)`, so `Edit ▸ 撤销` routes through `ModuleCodeEditorCommands` to the focused editor and falls back to the responder chain for ordinary text fields.

Find highlighting uses `NSLayoutManager` temporary attributes rather than `textStorage` attributes, so the syntax-highlight pass cannot erase it and module content is never modified. The editor deliberately stays outside `NSScrollView` (see the note in `ModuleCodeTextView.swift`), so the system find bar cannot be used and `ModuleCodeSearchBar` provides find/replace instead; keep `usesFindPanel` off so no dead system Find path remains.
...
Diagnostic payloads and snapshots live in focused files under `Models/`: report payloads in `DiagnosticReportModels.swift`, installation and crash diagnostics in `InstallationDiagnosticSnapshot.swift`, local credential diagnostics in `CredentialDiagnostics.swift`, and local-root diagnostics in `LocalModuleRootDiagnosticSnapshot.swift`. Keep diagnostic model changes there instead of adding another shared catch-all model file.
