...
Web management markup helpers live in `WebResources/web-markup.js`. Keep reusable HTML fragments such as sidebar module rows, empty states, detail rows, copyable URL/value sections, preview shells, total-module and single-module detail sections, detail toolbars, argument controls, advanced option rows, latest-publish sections, and publish-file lists there; `app.js` should compose those fragments with live state instead of owning their escaping details.
