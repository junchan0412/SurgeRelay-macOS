...
- 原生查找与批量替换在后台运行，长文档行号按可见区域绘制；查找最多高亮前 5,000 处，全部替换覆盖整篇文档。
...
| 跳转到行 | <kbd>⌘</kbd> + <kbd>L</kbd> |
| 切换注释 | <kbd>⌘</kbd> + <kbd>/</kbd> |
| 保存模块内容 | <kbd>⌘</kbd> + <kbd>S</kbd> |
...
- 监听本地模块目录：自写本地模块的源文件在 App 外被修改（编辑器、iOS 端 iCloud 同步）后会自动重新转换并刷新输出；判定依据是来源内容 sha256，只处理真正改动过的模块。App 未运行期间的改动会在启动后补扫。
- 模块内容编辑器提供撤销 / 重做、查找与替换（区分大小写、正则表达式、匹配计数、全部替换）、跳转到行、切换注释和缩进操作。
...
按 ⌘ / Ctrl + K 或在非输入区域按 `/` 可聚焦模块搜索；列表支持方向键、Home / End，实时更新会保留焦点。内容页支持 ⌘ / Ctrl + S 写入，并显示载入、未保存、写入中和读取失败重试状态。保存或恢复期间继续输入的内容会保留为草稿；较大的总模块采用完整纯文本预览，仍可拷贝全部内容。
