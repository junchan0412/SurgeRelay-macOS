...
此 fork 基于原项目的 Apache License 2.0 许可进行修改，保留原许可证与第三方声明。当前 fork 增加了本地/GitHub 模块根目录、每个模块的 Surge `category` 标签，以及按根目录子文件夹保存独立转换模块的能力。
...
在 [Releases](https://github.com/junchan0412/SurgeRelay-macOS/releases) 下载 `Surge-Relay-*.app.zip`，解压后将 `Surge Relay.app` 拖入“应用程序”文件夹即可。

当前 fork 的安装包为 ad-hoc 签名，尚未使用 Developer ID 公证。首次打开时如遇到 macOS 安全提示，请在“系统设置”>“隐私与安全性”中允许打开，或参考下方“App 已损坏”说明移除隔离属性。
