...
- 模块关系明确分为“模块存放位置”和“转换前来源”：模块可以存放在本地或 GitHub，转换前来源可以是远程 QX/Loon/Surge，也可以是纯本地 Surge 文件。
- 本地发布和 GitHub 发布可以同时开启；本地根目录和 GitHub 模块目录使用同一套输出文件夹逻辑。
...
扫描本地根目录时，App 会读取已有 `.sgmodule` 的 `#!name`、`#!category` 和 Script-Hub `#SUBSCRIBED` 来源记录，并在导入前展示预览。如果文件原本是 Script-Hub 转换结果，App 会把模块来源恢复为 `#SUBSCRIBED` 中的原始脚本地址，并同步来源格式、Script-Hub 参数和模块标签；如果没有这类记录，来源才会保存为本地 `file://` 文件。原文件仍留在原位置，不会被复制到另一个同名位置。

左侧模块列表按维护状态分组：

- “需要处理”：最近更新失败或本地编辑与上游更新发生冲突。
- “本地模块”：转换后存放在本地模块根目录；其中既可能是纯本地 Surge 文件，也可能是带远程原始来源的本地模块。
- “GitHub 模块”：转换后存放到 GitHub 模块目录；转换前来源仍可独立显示为 QX、Loon 或 Surge。
- “未分类”：来源无法解析，需要编辑模块补充原始地址后再更新。

模块详情页和 Web 管理端都会优先显示“管理关系”：模块存放、转换前来源、原始地址、来源记录和本地相对路径会分开展示。判断一个模块是否会管理你的原文件时，请看“模块存放”和“本地相对路径”；判断它能否从上游更新时，请看“转换前来源”和“原始地址”。
...
当前工程仍保留 `NSAllowsArbitraryLoads=true` 和关闭 App Sandbox。原因是 Surge Relay 需要转换用户添加的任意 HTTP/HTTPS 模块来源，并写入用户选择的本地 Surge/iCloud 目录或执行 GitHub 发布；若开启沙盒，需要进一步引入 security-scoped bookmark 并迁移现有本地目录访问模型。这两项会继续作为后续硬化方向跟踪。
