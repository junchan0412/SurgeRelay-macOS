...
模块列表右键菜单提供“编辑 / 更新 / 在访达中显示（本地模块）/ 删除”。删除本地独立模块时，会一并真实删除其受 Surge Relay 管理的输出文件；自写模块的用户源文件不属于管理对象，会安全保留。
...
```bash
VERSION=1.3.34 BUILD=83 ./script/check_release_configuration.sh
```
