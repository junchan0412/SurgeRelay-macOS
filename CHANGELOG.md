...
- 工作台与详情：指标在窄窗口自动调整布局，发布去向区分未开启与待配置，明确显示总模块参与关系、冲突类型和不可更新原因；侧栏直接展示搜索与筛选条件，支持一键重置并补齐无障碍标签。
- 活动记录：新增后台即时搜索，覆盖模块名、结果、详情、commit 与发布文件名；支持展开文件变更、拷贝完整记录，清空确认明确显示条数及搜索之外的清空范围。
- Web 交互：支持搜索快捷键、方向键 / Home / End 列表导航，实时更新保留焦点并复用未变动行；移动端改进工具栏、触控目标、输入字号和弹窗取消行为。
- 草稿与恢复：Web 内容页明确显示未保存、写入中和失败重试状态，支持 ⌘ / Ctrl + S，保存或恢复期间继续输入不会被旧响应覆盖；服务端阻止重复写入，缺失转换缓存时保留手动覆盖。
- 原生编辑器：查找与批量替换移到可取消的后台任务，隔离过期结果；行号位图只绘制可见区域，复用行索引与尺寸缓存，修复重复排版引起的滚动回跳，降低长文档滚动与编辑的开销。
- 查找替换：修复普通“全部替换”超过 5,000 处时漏改、正则 lookaround 捕获替换与选区锚点错误；高亮上限不再限制批量替换范围。
- 搜索一致性：更改模块名称、分类、注释、图标、订阅元数据或双发布目标后，内容搜索与列表索引及时失效，避免旧结果残留。
- 输出可靠性：部分更新遇到缺缓存或本地导出失败时保留错误状态，避免误报完成；切换本地发布根目录后不再复用旧目录的覆盖权限或受管文件清单。
- 性能验证：4.872 MB 文本、4,800 处普通替换的本机 Release 优化基准，5 次中位数由 197.89 ms 降到 17.87 ms，输出逐字一致；该数据为受控微基准，实际耗时随内容与设备变化。
- 验证：362 项 Swift 单元测试及 Web 行为 / DOM 测试通过，完成深浅色、活动搜索、侧栏操作与 12,000 行滚动 / 替换 / 撤销的桌面验证。原生 UI test target 构建通过；当前 macOS automation mode 超时，XCTest UI runner 未执行，相关流程已通过桌面操作补验。

## 2.1.0

- 外观：新增浅色 / 深色 / 跟随系统的主题选择（设置 › 通用），选择即时应用到窗口与菜单栏并持久保存；「跟随系统」恢复由 macOS 决定。
- 活动记录：在「需要关注」筛选下，「清空」仅清除更新失败与缓存回退记录，其他活动记录保留；「全部活动」下仍清空全部。
- 模块编辑：视图编辑器新增「注释」字段，写入 Surge 模块的 `#!desc=`（留空则保留来源自带说明），与显示名称、模块标签、图标一并覆盖到输出。
- 验证：Debug / Release 构建、代码签名与 Sparkle 签名通过；改动面单元测试（元数据、预览内容、发布组装、草稿规划、设置）全部通过。

## 2.0.1

- 视觉：中和了各区域背景的青绿色偏色，画布、卡片与描边统一为中性冷灰，与系统侧栏、材质和代码编辑区自然衔接，显著降低区域之间的明显色差；品牌青绿色收敛为唯一强调色。
- 视觉：统一全应用卡片样式——汇总详情、本地模块导入等页面的半透明材质卡片改为与详情 / 设置一致的实心卡片；侧栏底部状态区移除叠加材质，改为贴合侧栏的分隔线页脚。
- 视觉：拷贝按钮改用品牌强调色，与主操作按钮保持一致，不再回落到系统蓝。
- 验证：Debug / Release 构建、代码签名与 Sparkle 签名通过；浅色与深色外观均已核对。

## 2.0.0

- UI/UX：重做 macOS 与 Web 工作空间，新增工作台、独立活动记录、发布去向与集中问题入口；详情页采用“来源 → 转换 → 输出”的层级，技术信息按需展开。
- UI/UX：设置改为侧栏分类，工作台可直达发布配置；模块编辑优先呈现来源与输出，收起可选图标设置，补齐浅色 / 深色、手机布局、键盘焦点和减少动态效果支持。
- 性能：更新改为最多 4 个模块的有界并发流水线，完成顺序不影响合并顺序；原生模块复用来源检查响应，避免内容变化后再次下载。
- 性能：模块汇总按数据修订失效，搜索分组移到后台，搜索键不再反复 JSON 编码；Web 模块投影复用缓存，进度接口不再构建列表或读取图标，资源指纹改为流式 SHA-256。
- 稳定性：转换内容与脚本资源作为同一快照原子提交，保留旧缓存与手动覆盖；取消会停止排队和网络工作、收尾状态并保存已完成结果，启动时恢复中断状态。
- 稳定性：统一网络连接复用、响应大小限制、任务取消和受限请求重定向校验；Web 连接加入退避重连、隐藏页面暂停、过期响应隔离和单次轮询约束。
- 编辑：两端内容页在切换模块时保留草稿；保存使用提交时的文本快照，保存期间继续输入不会被误标记为已保存。
- 发布：恢复 Swift 6、完整并发检查与 `-O` Release 优化，移除发布包的 Debug dylib；保留现有固定签名证书、Sparkle 更新链、配置与发布地址兼容性。
- 发布：补齐 Xcode 漏登记的 Web 详情脚本，发布验证直接检查包内 HTML 引用的资源及脚本语法，防止源码正常而安装包缺失资源。
- 验证：330 项 Swift 测试及 Web 行为 / DOM 测试通过。24 个各等待 25 ms 的本机流水线基准由 0.637 秒降至 0.157 秒；这是受控延迟基准，真实耗时取决于来源和转换任务。

## 1.4.7

- 修复：本地模块源文件在磁盘上被修改后不再需要等待刷新间隔或手动更新，App 会监听本地模块目录（FSEvents），比较来源内容 sha256 后只重新转换真正改动过的模块并刷新输出；App 未运行期间发生的改动会在启动后补扫同步。
- 设置：新增“同步本地模块改动”开关（默认开启）与“立即检查”按钮，用于随时比对全部本地模块源文件。
- 文本编辑：修复 ⌘Z / ⇧⌘Z 无效的问题。缩进、反缩进、保持缩进换行、注释切换和查找替换现在都进入 NSTextView 的撤销栈，编辑器也拥有独立的撤销管理器。
- 文本编辑：新增查找与替换栏（⌘F / ⌥⌘F），支持区分大小写、正则表达式、上一个/下一个（⌘G / ⇧⌘G）、匹配计数、单个替换与全部替换（整篇替换合并为一次撤销）。
- 文本编辑：新增跳转到行（⌘L）、切换注释（⌘/），以及界面上可见的撤销、重做和查找按钮；主菜单补充对应的编辑命令。
- 文本编辑：独立文本编辑器窗口的“完成”不再占用 return 键，⌘W 关闭，return 恢复为换行。
- 验证：常规 Xcode 单元测试、Debug build、Release 构建、代码签名、Sparkle 签名、appcast 同步与发布资产校验通过。
