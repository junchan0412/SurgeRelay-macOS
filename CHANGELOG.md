...
- 将 Web 管理端侧栏渲染、失败筛选和列表 live patch 拆入 `web-sidebar.js`，让 `app.js` 更专注于状态流、详情页和用户动作编排。
...
- 将模块详情顶部摘要卡拆出到 `ModuleDetailSummaryHeader.swift`，让 `ModuleDetailView.swift` 更专注于管理关系、同步状态、参数和发布信息。
...
- 将本地发布受管理路径、旧文件清理预览和根目录变化保护抽出到 `LocalPublishedFilesPlanner`，避免 AppModel 内联判断误扩散到用户原有模块。
...
- 将通用和发布设置页拆出到独立视图，让 Script-Hub 设置、本地/GitHub 发布配置和本地根目录诊断脱离设置窗口外壳。
...
- 将更新完成后的“等待发布 / 等待清理 / 已刷新”状态文案和自动发布调度动作抽出到 `UpdateCompletionStatusPlanner`，让失败缓存沿用提示与收尾调度可被独立测试锁定；开始新一轮更新时也会清掉旧自动发布排队时间，避免界面残留。
